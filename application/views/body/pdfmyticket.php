		...
		<thead>
                           ...
                           <?php $no = 0; foreach($datamyticket as $row) : $no++;?>
						     <tr>
						        ...
						        <td data-field="id"><?php if($row->status==1) { echo "WAITING APPROVAL";}
						        else if($row->status==2) { echo "APPROVED";}
						        else if($row->status==0) { echo "TICKET REJECTED";}
...
<a title="Hapus Kontak" class="hapus btn btn-danger btn-xs" href="<?php echo base_url();?>myticket/feedback_no/<?php echo $row->id_ticket;?>/<?php echo $row->id_teknisi;?>"><span class="glyphicon glyphicon-thumbs-down"></span></a>
									<?php } else if($row->status==6 AND  $row->feedback == 1) { echo "POSITIVE FEEDBACK FROM YOU";}
										  else if($row->status==6 AND $row->feedback == 0) { echo "NEGATIVE FEEDBACK FROM YOU ";}
										  else
										  {
										  	echo "WAITING STATUS SOLVED FROM TECHNICIAN";
										  }
									?>
						        </td>
						    </tr>
